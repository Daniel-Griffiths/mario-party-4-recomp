...
void BootExec(void)
{
    ...
    repeat:
        ...
        if (!SystemInitF) {
            OSReport("[BOOT] first wipe done, starting init\n");
            tick_prev = OSGetTick();
            OSReport("[BOOT] CharManInit...\n");
            CharManInit();
            OSReport("[BOOT] HuWindowInit...\n");
            HuWindowInit();
            OSReport("[BOOT] MGSeqInit...\n");
            MGSeqInit();
            OSReport("[BOOT] HuWinInit...\n");
            HuWinInit(1);
            OSReport("[BOOT] BootTitleCreate...\n");
            BootTitleCreate();
            OSReport("[BOOT] HuSprAnimReadFile(HUDSON)...\n");
            data = HuSprAnimReadFile(TITLE_HUDSON_ANM);
            OSReport("[BOOT] HuSprCreate(hudson)... data=%p\n", data);
            sprite_hudson = HuSprCreate(data, 0, 0);
            HuSprGrpMemberSet(group, 1, sprite_hudson);
            HuSprPosSet(group, 1, 288, 240);
            HuSprAttrSet(group, 1, HUSPR_ATTR_DISPOFF);
            OSReport("[BOOT] waiting 3sec tick...\n");
            while (OSTicksToMilliseconds(OSGetTick() - tick_prev) < 3000) {
                HuPrcVSleep();
            }
            OSReport("[BOOT] 3sec wait done\n");
        }
        else {
            for (i = 0; i < 180; i++) {
                #if VERSION_NTSC
        ...
        OSReport("[BOOT] fade out Nintendo...\n");
        WipeCreate(WIPE_MODE_OUT, WIPE_TYPE_NORMAL, 30);
        while (WipeStatGet()) {
            HuPrcVSleep();
        }
        OSReport("[BOOT] show Hudson logo...\n");
        HuSprAttrSet(group, 0, HUSPR_ATTR_DISPOFF);
        HuSprAttrReset(group, 1, HUSPR_ATTR_DISPOFF);
        HuPrcSleep(60);
        OSReport("[BOOT] fade in Hudson...\n");
        WipeCreate(WIPE_MODE_IN, WIPE_TYPE_NORMAL, 30);
        while (WipeStatGet()) {
            HuPrcVSleep();
        }
        OSReport("[BOOT] Hudson fade done\n");
        if (!SystemInitF) {
            void *group_samp;
            OSReport("[BOOT] audio load + 3sec wait...\n");
            tick_prev = OSGetTick();
        ...
        skip_wait = FALSE;
        OSReport("[BOOT] THP section done, looping to repeat\n");
    }
    OSReport("[BOOT] calling BootTitleExec\n");
    if (!BootTitleExec()) {
        HuPrcSleep(60);
        goto repeat;
    }
...
#ifdef TARGET_PC
    /* nintendoData is a byte array with big-endian header fields */
    u32 size = (nintendoData[0] << 24) | (nintendoData[1] << 16) | (nintendoData[2] << 8) | nintendoData[3];
    int decode_type = (nintendoData[4] << 24) | (nintendoData[5] << 16) | (nintendoData[6] << 8) | nintendoData[7];
    src += 2;
#else
    u32 size = *src++;
    int decode_type = *src++;
