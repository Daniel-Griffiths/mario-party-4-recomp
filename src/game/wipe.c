...
static int wipe_dbg_count = 0;
void WipeExecAlways(void)
{
	s32 i;
	WipeState *wipe;
	wipe = &wipeData;
	if (wipe->mode != 0 && wipe_dbg_count < 200) {
		if (wipe_dbg_count % 10 == 0)
			OSReport("[WIPE] mode=%d stat=%d time=%.1f dur=%.1f\n", wipe->mode, wipe->stat, wipe->time, wipe->duration);
		wipe_dbg_count++;
	} else if (wipe->mode == 0 && wipe_dbg_count > 0 && wipe_dbg_count < 200) {
		OSReport("[WIPE] completed (mode=0 stat=%d)\n", wipe->stat);
		wipe_dbg_count++;
	}
	switch(wipe->mode) {
...
{
#ifdef TARGET_PC
	/* On PC, wipe is drawn as an SDL overlay in vi_pc.c after sprites.
	 * Skip the GX rendering to avoid double-draw into the 3D framebuffer. */
	(void)color;
	return;
#else
