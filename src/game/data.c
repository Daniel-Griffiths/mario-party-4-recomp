...
#ifdef TARGET_PC
#define PTR_OFFSET(ptr, offset) (void *)(((u8 *)(ptr)+(uintptr_t)(offset)))
#else
...
#ifdef TARGET_PC
    {
        /* Data file offsets and header fields are big-endian */
        u8 *be = (u8 *)temp_ptr;
        u32 offset = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
        read_stat->file = PTR_OFFSET(read_stat->dir, offset);
        be = (u8 *)read_stat->file;
        read_stat->raw_len = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
        read_stat->comp_type = (be[4] << 24) | (be[5] << 16) | (be[6] << 8) | be[7];
        read_stat->file = (u8 *)read_stat->file + 8;
    }
#else
...
static void *HuDataDecodeIt(void *buf_start, s32 buf_ofs, s32 num, HeapID heap)
{
	...
	if((uintptr_t)buf & 0x3) {
		...
		s32 *data = buf;
#ifdef TARGET_PC
		/* Data files are big-endian; byte-swap on PC */
		u8 *be = (u8 *)data;
		raw_len = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
		comp_type = (be[4] << 24) | (be[5] << 16) | (be[6] << 8) | be[7];
		data += 2;
#else
		raw_len = *data++;
		comp_type = *data++;
...
void *HuDataReadNumHeapShortForce(s32 data_id, s32 num, HeapID heap)
{
	...
	{
		u8 *be = (u8 *)file_data;
		file_max = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
	}
#else
	file_max = *file_data;
#endif
	...
	{
		u8 *be = (u8 *)data_hdr;
		file_ofs = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
	}
#else
	file_ofs = *data_hdr;
#endif
	...
	if(file_max <= file_id+1) {
		...
		{
			u8 *be = (u8 *)data_hdr;
			s32 next_ofs = (be[0] << 24) | (be[1] << 16) | (be[2] << 8) | be[3];
			data_ofs = next_ofs - read_ofs;
		}
#else
