...
#ifdef TARGET_PC
/* Guard macros for functions taking a model index — skip if invalid (e.g. -1 from failed load) */
#define HU3D_MDL_GUARD(idx) do { if ((idx) < 0 || (idx) >= HU3D_MODEL_MAX) return; } while(0)
#define HU3D_MDL_GUARD_V(idx, val) do { if ((idx) < 0 || (idx) >= HU3D_MODEL_MAX) return (val); } while(0)
#else
#define HU3D_MDL_GUARD(idx) ((void)0)
#define HU3D_MDL_GUARD_V(idx, val) ((void)0)
#endif
...
void Hu3DExec(void) {
    ...
    HuSprDispInit();
#ifdef TARGET_PC
    /* Flush the GX framebuffer (3D content) to SDL between background
     * sprites (draw_no=0x7F, already drawn) and foreground sprites
     * (draw_no=0, about to be drawn). This matches GC behavior where
     * both sprites and 3D share the same EFB. */
    {
        extern void GXPCFlushFramebuffer(void);
        GXPCFlushFramebuffer();
    }
#endif
    HuSprExec(0);
    data = Hu3DData;
...
s16 Hu3DModelCreate(void *arg0) {
...
#ifdef TARGET_PC
    if (!arg0) {
        OSReport("Hu3DModelCreate: NULL data, skipping\n");
        return -1;
    }
#endif
    var_r31 = Hu3DData;
...
#ifdef TARGET_PC
    var_r31->unk_48 = Hu3DMallocNo = (u32)(uintptr_t)var_r31->hsfData;
#else
    ...
    var_r31->unk_64 = 0.0f;
#ifdef TARGET_PC
    /* On PC, HSF loading is stubbed — skip all HSF field accesses */
    if (!var_r31->hsfData->root) {
        ...
        var_r31->attr |= HU3D_ATTR_DISPOFF;
...
s16 Hu3DModelLink(s16 arg0) {
    ...
    HU3D_MDL_GUARD_V(arg0, -1);
    temp_r30 = &Hu3DData[arg0];
#ifdef TARGET_PC
    /* If source model has empty HSF data, create a linked model with DISPOFF */
    if (!temp_r30->hsfData || !temp_r30->hsfData->root) {
        var_r31 = Hu3DData;
        for (var_r28 = 0; var_r28 < HU3D_MODEL_MAX; var_r28++, var_r31++) {
            if (var_r31->hsfData == 0x0) break;
        }
        if (var_r28 == HU3D_MODEL_MAX) return -1;
#ifdef TARGET_PC
        ...
        *var_r31->hsfData = *temp_r30->hsfData;
        ...
        var_r31->camera = -1; var_r31->layer = 0;
        var_r31->unk_120 = 0; var_r31->unk_26 = 0; var_r31->unk_03 = 0; var_r31->unk_02 = 0;
        var_r31->unk_58.x = var_r31->unk_58.z = var_r31->unk_58.y = 1.0f;
        var_r31->unk_04 = -1; var_r31->unk_24 = arg0;
        for (i = 0; i < 8; i++) { var_r31->unk_38[i] = -1; }
        var_r31->unk_01 = 0;
        var_r31->unk_00 = (u8) var_r28;
        PSMTXIdentity(var_r31->unk_F0);
        layerNum[0] += 1;
        return var_r28;
    }
...
BOOL Hu3DModelCameraInfoSet(s16 arg0, u16 arg1) {
    ...
    for (i = 0; i < temp_r27->objectCnt; i++, var_r23++) {
        obj_copy = var_r23;
        if (obj_copy->type == 7) {
            /* On GC (32-bit), data.base.rot.x aliases camera.aspect_dupe
             * at the same union offset (24). On 64-bit PC, pointers are
             * larger so data.base.rot.x shifts to offset 36 (camera.far).
             * Read the correct field directly on PC. */
#ifdef TARGET_PC
