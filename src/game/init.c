...
#ifdef TARGET_PC
    /* On PC, arena is a malloc'd block. Allocate framebuffers from it.
     * Use uintptr_t for pointer arithmetic to avoid 64-bit truncation. */
    u32 fb_size = 640 * 480 * 2; /* YUV framebuffer size approximation */
    DemoFrameBuffer1 = (void *)(((uintptr_t)arena_lo + 31) & ~(uintptr_t)31);
    DemoFrameBuffer2 = (void *)(((uintptr_t)DemoFrameBuffer1 + fb_size + 31) & ~(uintptr_t)31);
    DemoCurrentBuffer = DemoFrameBuffer2;
    arena_lo = (void *)(((uintptr_t)DemoFrameBuffer2 + fb_size + 31) & ~(uintptr_t)31);
    OSSetArenaLo(arena_lo);
    arena_lo = OSGetArenaLo();
    arena_hi = OSGetArenaHi();
    arena_lo = OSInitAlloc(arena_lo, arena_hi, 1);
    OSSetArenaLo(arena_lo);
    arena_lo = (void *)(((uintptr_t)arena_lo + 31) & ~(uintptr_t)31);
    arena_hi = (void *)((uintptr_t)arena_hi & ~(uintptr_t)31);
