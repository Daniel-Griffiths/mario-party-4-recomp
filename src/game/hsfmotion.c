...
#ifdef TARGET_PC
/* Check if HsfData from a MotionData is valid (non-NULL with valid motion) */
#define MOTION_HSF_VALID(md) ((md)->unk_04 && (md)->unk_04->motion)
/* Check if a ModelData's hsfData has valid root/object data */
#define MODEL_HSF_VALID(md) ((md)->hsfData && (md)->hsfData->root)
#endif
...
#ifdef TARGET_PC
            if (MODEL_HSF_VALID(temp_r31) && Hu3DMotion[arg1].unk_04 && Hu3DMotion[arg1].unk_04->root)
#endif
...
void Hu3DMotionTimeSet(s16 arg0, float arg1) {
    ...
    if (temp_r31->hsfData != (HsfData*) -1
#ifdef TARGET_PC
        && MODEL_HSF_VALID(temp_r31)
#endif
        && temp_r31->hsfData->cenvCnt != 0 && (temp_r31->motion_attr & HU3D_MOTATTR_PAUSE)) {
...
#ifdef TARGET_PC
    if (!MOTION_HSF_VALID(temp_r30)) return 0.0f;
#endif
...
#ifdef TARGET_PC
    if (!temp_r31->unk_04->motion) return 0.0f;
#endif
...
#ifdef TARGET_PC
    if (!MOTION_HSF_VALID(temp_r27)) return;
#endif
...
#ifdef TARGET_PC
    if (!MOTION_HSF_VALID(sp18) || !MODEL_HSF_VALID(temp_r27)) return;
#endif
...
#ifdef TARGET_PC
    if (!MOTION_HSF_VALID(temp_r28)) return;
#endif
...
#ifdef TARGET_PC
    if (!MOTION_HSF_VALID(temp_r22) || !MODEL_HSF_VALID(temp_r30)) return;
#endif
...
void JointModel_Motion(s16 arg0, s16 arg1) {
    ...
    if (!temp_r24->hsfData || !temp_r24->hsfData->root) return;
    if (!temp_r23->unk_04 || !temp_r23->unk_04->motion) return;
#endif
    HsfMotion *temp_r29;
    HsfTrack *var_r30;
    HsfTrack *var_r28;
