...
def clear_xattr(path) -> None:
    if platform.system() == "Darwin" and shutil.which("xattr"):
        subprocess.call(["xattr", "-cr", str(path)], stderr=subprocess.DEVNULL)


def download_with_curl(url, output) -> None:
...
def main() -> None:
    ...
    # Use curl as primary download method (more reliable than urllib on Python 3.14)
